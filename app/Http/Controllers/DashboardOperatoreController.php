    ...
    public function index(Request $request)
    {
        ...
        $fasiVisibili = OrdineFase::where(function ($q) use ($reparti, $releaseDef2) {
                ...
                $q->orWhere(function ($q2) use ($releaseDef2) {
                    $q2->where('stato', 3)
                        ->where(function ($qs) {
                            // Fase ancora da gestire: scarti mancanti O scarico mancante
                            $qs->whereNull('scarti')
                               ->orWhere('scarti', 0)
                               ->orWhere(function ($q3) {
                        ...
                        // Prelievo unico per commessa: se sibling ha gia scaricato,
                        // la fase si autotermina senza richiedere nulla (esce da dashboard)
    ...
    public function prestampa(Request $request)
    {
        ...
        $commesse = \App\Models\Ordine::whereHas('fasi', fn($q) => $q->where('stato', '<', 3))
            ->select('commessa',
                ...
                \Illuminate\Support\Facades\DB::raw("SUM(CASE WHEN cod_art NOT LIKE 'SEMILAV%' THEN qta_richiesta ELSE 0 END) as qta_richiesta"))
            ->groupBy('commessa')
