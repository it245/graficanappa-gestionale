...
<div class="mes-sidebar-section">
    ...
    <a href="{{ route('owner.costi.analisi.index') }}?op_token={{ request('op_token') }}" class="mes-sidebar-item">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Forza placeholder visibile sull'input search di Choices.js multi
    function setPlaceholder(id, txt) {
        const el = document.getElementById(id);
        if (!el) return;
        const wrap = el.closest('.choices');
        if (!wrap) return;
        const input = wrap.querySelector('.choices__input');
        if (input) input.setAttribute('placeholder', txt);
    }
    setPlaceholder('filterStato', 'Seleziona Stato');
    setPlaceholder('filterFase', 'Seleziona Fase');
    setPlaceholder('filterReparto', 'Seleziona Reparto');
