...
<tr id="fase-{{ $fase->id }}" class="{{ $rowClass }}">
    ...
    <td class="td-commessa">
        ...
        @if(in_array($repNomeEtichetta, ['finestratura', 'legatoria', 'piegaincolla', 'tagliacarte', 'digitale']))
        <a href="{{ route('operatore.etichetta', $fase->ordine->id) }}?fase={{ $fase->id }}" class="ms-1"
