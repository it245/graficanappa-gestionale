                        ...
                        @if($c->ordini->count() > 1)
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a class="dropdown-item" href="{{ route('etichetta.show', $ord->id) }}?from=etichette" style="font-size:12px;">{{ Str::limit($ord->descrizione, 45) }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                        @else
                            <a href="{{ route('etichetta.show', $c->ordini->first()->id) }}?from=etichette" class="btn btn-sm btn-primary" style="font-size:12px;">Stampa</a>
                        @endif
                    </td>
                </tr>
                @empty
