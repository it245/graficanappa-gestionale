...
foreach ($perReparto as $reparto => $fasirep) {
    $vecchie = $fasirep->filter(fn($f) => $f->data_inizio && abs(now()->diffInDays(\Carbon\Carbon::parse($f->data_inizio))) > 3);
    echo "--- {$reparto}: {$fasirep->count()} fasi ({$vecchie->count()} aperte da >3gg) ---\n";
    foreach ($fasirep as $f) {
        $giorni = $f->data_inizio ? abs(now()->diffInDays(\Carbon\Carbon::parse($f->data_inizio))) : '?';
...
$vecchieTot = $fasi->filter(fn($f) => $f->data_inizio && abs(now()->diffInDays(\Carbon\Carbon::parse($f->data_inizio))) > 3);
echo "Aperte da piu di 3 giorni: {$vecchieTot->count()}\n";
echo "Aperte da piu di 7 giorni: " . $fasi->filter(fn($f) => $f->data_inizio && abs(now()->diffInDays(\Carbon\Carbon::parse($f->data_inizio))) > 7)->count() . "\n";
