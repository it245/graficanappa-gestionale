...
<div class="container mt-3">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            ...
            <p class="text-muted mb-0">Benvenuto, <?php echo e(request()->attributes->get('operatore_nome') ?? session('operatore_nome')); ?></p>
        </div>
        <div>
            <a href="#" class="btn btn-success me-2" data-bs-toggle="modal" data-bs-target="#aggiungiOperatoreModal">+ Nuovo operatore</a>
            <a href="<?php echo e(route('admin.statistiche')); ?>" class="btn btn-info me-2">Statistiche</a>
            <a href="<?php echo e(route('admin.commesse')); ?>" class="btn btn-primary me-2">Storico Commesse</a>
            <a href="<?php echo e(route('admin.reportProduzione')); ?>" class="btn btn-warning me-2">Report Settimanale</a>
            <a href="<?php echo e(route('admin.cruscotto')); ?>" class="btn btn-danger me-2">Cruscotto Direzionale</a>
            <a href="<?php echo e(route('admin.reportDirezione')); ?>" class="btn btn-dark me-2">Report Direzione</a>
            <a href="<?php echo e(route('admin.reportPrinect')); ?>" class="btn btn-secondary me-2">Report Prinect</a>
            <a href="<?php echo e(route('admin.costi.report')); ?>" class="btn btn-outline-success me-2">Costi & Margini</a>
            <a href="<?php echo e(route('mes.fiery')); ?>" class="btn btn-outline-warning me-2">Fiery V900</a>
    ...
    <div class="card">
        <div class="card-header bg-dark text-white">
            <strong>Operatori (<?php echo e($operatori->count()); ?>)</strong>
        </div>
        <div class="card-body p-0">
            <table class="table table-bordered table-sm table-striped mb-0" style="font-size:13px;">
                ...
                <tbody>
                    ...
                    <tr class="<?php echo e(!$op->attivo ? 'table-secondary' : ''); ?>">
                        <td><strong><?php echo e($op->codice_operatore); ?></strong></td>
                        <td><?php echo e($op->nome); ?></td>
                        <td><?php echo e($op->cognome); ?></td>
                        <td>
                            <?php if($op->ruolo === 'admin'): ?>
                                <span class="badge bg-dark">admin</span>
                            <?php elseif($op->ruolo === 'owner'): ?>
                                <span class="badge bg-primary">owner</span>
                            <?php else: ?>
                                <span class="badge bg-secondary">operatore</span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo e($op->reparti->pluck('nome')->join(', ') ?: '-'); ?></td>
                        ...
                        <td>
                            <a href="<?php echo e(route('admin.operatore.modifica', $op->id)); ?>" class="btn btn-sm btn-outline-primary">Modifica</a>
                            <form method="POST" action="<?php echo e(route('admin.operatore.toggleAttivo', $op->id)); ?>" style="display:inline;">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="btn btn-sm <?php echo e($op->attivo ? 'btn-outline-warning' : 'btn-outline-success'); ?>">
                                    <?php echo e($op->attivo ? 'Disattiva' : 'Attiva'); ?>
    ...
    <div class="card mt-4">
        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
            <strong>Codici EAN Prodotti (<?php echo e($eanProdotti->count()); ?>)</strong>
        ...
        <div class="card-body p-0">
            <table class="table table-bordered table-sm table-striped mb-0" style="font-size:13px;">
                ...
                <tbody>
                    <?php $__currentLoopData = $eanProdotti; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ean): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr id="ean-row-<?php echo e($ean->id); ?>">
                        <td class="ean-display"><?php echo e($ean->articolo); ?></td>
                        <td class="ean-display"><?php echo e($ean->codice_ean); ?></td>
                        <td class="ean-display">
                            <button type="button" class="btn btn-sm btn-outline-primary btn-ean-edit" data-id="<?php echo e($ean->id); ?>">Modifica</button>
                            <form method="POST" action="<?php echo e(route('admin.ean.elimina', $ean->id)); ?>" style="display:inline;" onsubmit="return confirm('Eliminare questo codice EAN?')">
                                <?php echo csrf_field(); ?>
                        ...
                        <td class="ean-edit" style="display:none;" colspan="3">
                            <form method="POST" action="<?php echo e(route('admin.ean.aggiorna', $ean->id)); ?>" class="d-flex align-items-center gap-2">
                                <?php echo csrf_field(); ?>
                                <input type="text" name="articolo" value="<?php echo e($ean->articolo); ?>" class="form-control form-control-sm" required style="max-width:200px;">
                                <input type="text" name="codice_ean" value="<?php echo e($ean->codice_ean); ?>" class="form-control form-control-sm" required style="max-width:200px;">
                                <button type="submit" class="btn btn-sm btn-success">Salva</button>
                                <button type="button" class="btn btn-sm btn-secondary btn-ean-cancel" data-id="<?php echo e($ean->id); ?>">Annulla</button>
                            </form>
                        </td>
                    </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php if($eanProdotti->isEmpty()): ?>
...
<div class="modal fade" id="aggiungiEanModal" tabindex="-1" aria-labelledby="aggiungiEanModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="<?php echo e(route('admin.ean.salva')); ?>">
            <?php echo csrf_field(); ?>
...
<div class="modal fade" id="aggiungiOperatoreModal" tabindex="-1" aria-labelledby="aggiungiOperatoreModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="<?php echo e(route('admin.operatore.salva')); ?>">
            <?php echo csrf_field(); ?>
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="adminCodice" class="form-control" value="<?php echo e($prossimoCodice); ?>" data-numero="<?php echo e($prossimoNumero); ?>" disabled>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="reparto_principale" class="form-select" required>
                            <?php $__currentLoopData = $reparti; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $rep): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($id); ?>"><?php echo e($rep); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    <div class="mb-3">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Giovanni\graficanappa-gestionale\resources\views\admin\dashboard.blade.php ENDPATH**/ ?>