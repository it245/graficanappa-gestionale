...
{
    /**
     * Deduplica fasi digitale/finitura digitale: 1 sola per commessa + cod_art + fase_catalogo.
     * V2: usa cod_art (VARCHAR) invece di descrizione (TEXT) per il GROUP BY.
     */
    public function up(): void
    {
        ...
        // Trova duplicati per commessa + cod_art + fase_catalogo_id
        $duplicati = DB::table('ordine_fasi')
            ...
            ->groupBy('ordini.commessa', 'ordini.cod_art', 'ordine_fasi.fase_catalogo_id')
            ->having('cnt', '>', 1)
            ->get();
        ...
        foreach ($duplicati as $dup) {
            ...
            // Elimina le altre (solo se stato <= 1)
        ...
        if ($totaleEliminati > 0) {
            \Illuminate\Support\Facades\Log::info("Migration dedup digitale v2: eliminati {$totaleEliminati} duplicati");
        }
    }
