<?php
/**
 * Simulazione v6: token-subset match con scoring.
 *
 * Algoritmo:
 *  1. Tokenize cliché (dopo norm) e MES (dopo strip+norm)
 *  2. Match = TUTTI i token cliché ∈ token MES
 *  3. Score = numero token cliché (più alto = più specifico)
 *  4. Per ogni fase MES, prendi cliché con score MAX
 *  5. Cliché con < 2 token: solo match esatto stringa (no subset)
 */
...
// Parole comuni che non contano per matching (troppo generiche)
const STOPLIST = ['DI', 'LA', 'IL', 'E', 'DEL', 'DELLA', 'CON', 'AL', 'ALLA', 'DA'];

function tokenize(string $norm): array {
    $toks = preg_split('/\s+/u', trim($norm));
    return array_values(array_filter($toks, fn($t) => $t !== '' && !in_array($t, STOPLIST, true)));
}
...
// Statistiche token
$distr = [];
foreach ($cliche as $c) $distr[$c['ntok']] = ($distr[$c['ntok']] ?? 0) + 1;
ksort($distr);
echo "  Distrib. # token cliché: ";
foreach ($distr as $k => $v) echo "{$k}tok=$v ";
echo "\n";
...
foreach ($fasi as $f) {
    ...
    if (isset($clicheByNorm[$mesNorm])) {
        $bestMatch = $clicheByNorm[$mesNorm];
        $bestScore = $bestMatch['ntok'];
        $type = 'exact';
        $exact++;
    } else {
        // 2) token subset: cliché tokens ⊆ mes tokens
        foreach ($cliche as $c) {
            if ($c['ntok'] < 2) continue; // mono-token: solo exact
            $ok = true;
